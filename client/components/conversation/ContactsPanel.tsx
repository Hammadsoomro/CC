    ...
    const onNew = (e: any) => {
      ...
      setContacts((cs) => {
        const idx = cs.findIndex(
          (c) =>
            c.phoneNumber?.replace(/\s|\(|\)|-/g, "") === phone ||
            toE164(c.phoneNumber) === toE164(phone),
        );
        if (idx <= 0) return cs;
        const copy = cs.slice();
  ...
  const handleSelect = (c: ContactItem) => {
    ...
    if (count > 0) {
      ...
      window.dispatchEvent(
        new CustomEvent("sms:read", { detail: { phone: key, count } }),
      );
    }
  };
              ...
              <div className="flex items-center gap-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem
                      onClick={() => update(c._id, { pinned: !c.pinned })}
                    >
                      {c.pinned ? "Unpin" : "Pin"}
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() => update(c._id, { favorite: !c.favorite })}
                    >
        ...
        <DialogContent>
          ...
          <DialogFooter>
            <Button
              onClick={async () => {
                if (editing) {
                  await update(editing._id, { name: editName });
                  setEditOpen(false);
                  setEditing(null);
                }
              }}
