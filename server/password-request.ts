...
export const passwordRequestRoutes = {
  create: (async (req, res) => {
    ...
    if ((user.phone || "").trim() !== phone)
      return res.status(400).json({ error: "Details do not match" });
    if (
      firstName &&
      (user.firstName || "").trim().toLowerCase() !== firstName.toLowerCase()
    )
      return res.status(400).json({ error: "Details do not match" });
    if (
      lastName &&
      (user.lastName || "").trim().toLowerCase() !== lastName.toLowerCase()
    )
      return res.status(400).json({ error: "Details do not match" });

    const pr = await PasswordRequest.create({
      userId: user._id,
      email,
      phone,
      firstName,
